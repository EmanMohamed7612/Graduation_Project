...
import 'package:project_craftoria/feautures/Expert1/presentation/view/uploade_exepertdata.dart';

import '../../../auth/presentation/manager/cubit/auth_cubit.dart';
import '../manager/uploade_cubit.dart';
...
class _ExpertVerificationScreenState
            ...
            children: [
              ListTile(
                ...
                onTap: () async {
                  ...
                  if (img != null) {
                    setState(() {
                      profileImage = File(img.path); // ⭐ السطر الحاسم
                      uploadedFiles.add({
                        "name": img.name,
                        "type": "IMAGE",
                      });
              ...
              ListTile(
                ...
                onTap: () async {
                  ...
                  if (file != null) {
                    ...
                    setState(() {
                      portfolioFile = File(picked.path!); // ✅ هنا مكانه
                      uploadedFiles.add({
                        "name": picked.name,
                        "type": picked.extension!.toUpperCase(),
                      });
                    });
                  }
                }
  ...
  Widget build(BuildContext context) {
    ...
    SnackBar(content: Text("Files sent to backend successfully")),
    );
              ...
              GestureDetector(
                onTap: () {
                  if (uploadedFiles.length == 2 &&
                    ...
                    context.read<UploadCubit>().setProfileImage(profileImage!);
                    context.read<UploadCubit>().setPortfolioFile(portfolioFile!);

                    context.read<UploadCubit>().uploadFiles(
                      token: "USER_TOKEN_HERE", // ← توكن اليوزر
                    );
                  }
                },
