...
class CartCubit extends Cubit<CartState> {
  ...
  /// 🔹 Load Cart From Prefs (المعدلة)
  Future<void> loadCartFromPrefs() async {
    // شيلنا الـ emit(CartLoading()) عشان الـ Shimmer أو الـ Loader الرخم ما يظهرش وأنتِ راجعة بالسهم
    try {
      ...
      if (savedCartId != null && savedCartId.isNotEmpty) {
        ...
        // ✅ التعديل السحري: بنعمل emit لحالة ابتدائية عشان الـ Bloc Builder يحس إن الـ State اتغيرت فعلياً
        emit(CartInitial());

        // بعدين نبعت البيانات الجديدة فوراً
        emit(CartLoaded(cart));
      } else {
