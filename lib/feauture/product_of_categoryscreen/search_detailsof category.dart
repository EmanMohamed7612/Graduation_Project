...
// ✅ 1. تحديث استدعاءات الكيوبيت الموحد، الـ State، والموديل للمفضلة
// تأكدي من تعديل الحرف الناقص details إذا كان كودك كدة
...
class _Product2categoryScreenState extends State<Product2categoryScreen> {
  ...
  @override
  void dispose() {
    searchController.dispose(); // ✅ تنظيف الـ Controller لحماية الذاكرة
    super.dispose();
  }
...
class ProductCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: () {
        // ✅ 2. حفظ نسخة الكيوبيت الموحد وتمريره لشاشة التفاصيل لمنع انقطاع الـ Context
            ...
            child: Stack(
              children: [
                ...
                Positioned(
                  top: 8,
                  right: 8,
                  // ✅ 3. تعديل الـ BlocBuilder ليستقبل MyFavoriteState الجديد الموحد
                  child: BlocBuilder<MyFavoriteCubit, FavoriteState>(
                    builder: (context, state) {
                      final isFav = context.read<MyFavoriteCubit>().isFavorite(product.id);

                      return GestureDetector(
                        onTap: () {
                          // ✅ 4. تحويل البيانات لـ FavouriteModel كامل بدلاً من الـ id فقط لدعم الـ Cubit الجديد
