...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: [
        BlocProvider(
          create: (context) => ProductCubit(ProductApiService())..fetchTopProducts(),
        ),
        BlocProvider(
          create: (context) => BestSellerCubit(ProductApiService())..fetchBestSellers(),
        ),
        BlocProvider(
          create: (context) => SearchCubit(SearchApiService(DioClient().dio)),
        ),
      ],
      child: Scaffold(
        backgroundColor: AppColors.kBgColor,
...
// استيراد الـ Cubits والـ Services
import '../../../product_screens/manager/prodect_apiservice.dart';
import '../../../product_screens/manager/product_cubit.dart';
import '../../../product_screens/presentation/view/top_seller/manager/best_seller_cubit.dart';
import '../../../review/manager/cart_cubit.dart';
...
// استيراد الكيوبيت الجديد للمفضلة
import '../../manager/fav_apiserves.dart';
  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: [
        ...
        // 4. كيوبيت الكاتيجوري (إعادتها لأنها كانت ممسوحة في آخر نسخة)
        BlocProvider(
          create: (context) => CategoryCubit(ProductApiService())..fetchCategories(),
        ),
        // 5. كيوبيت السلة (إعادتها مع الـ Repo الخاص بها)
        // 5. كيوبيت السلة المطور (يقرأ ديناميكياً من الـ Prefs لمنع تضارب الـ IDs)
        BlocProvider(
          create: (context) => CartCubit(
            repo: CartRepo(),
            cartId: "1", // هيقرأ ديناميكياً
          )..loadCartFromPrefs(),
        ),
        // 6. ✅ إضافة الكيوبيت الجديد الموحد للمفضلة واستدعاء جلب البيانات فوراً
        BlocProvider(
          create: (context) => MyFavoriteCubit(FavoriteApiService(DioClient()))..getFavorites(),
        ),
      ],
      child: Scaffold(
        backgroundColor: AppColors.kBgColor,
