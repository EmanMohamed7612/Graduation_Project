            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  const Text(
                    'Enter Code',
                    style: TextStyle(color: Color(0xFF3E2723), fontSize: 22, fontWeight: FontWeight.bold),
                  ),
                  const SizedBox(height: 8),
                  Text(
                    "We sent a code to $email",
                    textAlign: TextAlign.center,
                    style: const TextStyle(color: Color(0xFF8D6E63), fontSize: 16),
                  ),
                  ...
                  state is AuthLoadingState
                    ...
                    onTap: () {
                      final otp = controllers.map((c) => c.text).join();
                      if(otp.length == 6) {
                        context.read<AuthCubit>().checkEmailOtp(email: email, otp: otp);
                      } else {
                        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Please enter complete code")));
                      }
                    },
                    child: Container(
